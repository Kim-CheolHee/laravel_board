...
<div class="container mx-auto p-8 bg-white shadow rounded">
       ...
       <form method="POST" action="{{ route('comments.store', ['bulletinBoard' => $post->bulletin_board_id, 'post' => $post]) }}" class="mt-4">
            @csrf
            ...
            @foreach ($post->comments as $comment)
                <div class="mt-4 border-t border-gray-300 pt-4">
                    ...
                    @if(auth()->id() === $comment->user_id)
                        ...
                        <button type="button" class="bg-red-400 text-white py-2 px-4 rounded hover:bg-red-600 mr-2" onclick="deleteComment({{ $comment->id }})">댓글 삭제</button>
